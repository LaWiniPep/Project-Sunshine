def main():
    ...
    while(keepGoing):
        ...
        response=input("Pick one (1/2) > " )
        print()
        if response=='1':
            nextNode=game[currentNode][2]
            currentNode=nextNode
            nextNode=()
            keepGoing=True
        else:
            nextNode=game[currentNode][4]
            currentNode=nextNode
            nextNode=()
            keepGoing=True
        if currentNode=="quit2":
            keepGoing=False
        else:
            keepgoing=True
...
game={    
    ...
    "sleep": ["A sudden crash outside the bedroom wakes you up again.", "1) Go and investigate the noise.", "wake2", "2) Go back to bed. That’s a tomorrow problem.", "quit1"], 
    ...
    "cotton": ["As you get close to the giants balls of cotton you realize they are sticky. When you check Inside you see Pepper and Lafayette are alive, but seem to be in a comatose state.", "1) Grab them and run out of the house!", "run", "2) Go back to bed. Things are getting to real.", "quit1"], 
    "cops": ["When you call 911 they laugh at you and ask if you have been drinking tonight.", "1) Yell at them tell them to send someone and  hang up.", "endCall", "2) Tell them you had been under the influence. And that you're going back to bed.", "quit1"], 
